...
def caption_audio_with_gemini(audio_path: str, previous_caption: Dict = None, model_name: str = "gemini-2.0-flash-exp") -> Dict:
    ...
    try:
        ...
        # 如果有前一個片段的資訊，加入 context
        prompt = PROMPT_TEXT
        if previous_caption:
            context = f"""
IMPORTANT CONTEXT - Previous segment characteristics:
- Genre: {previous_caption.get('suggested_genre', 'N/A')}
- Instrumentation: {previous_caption.get('instrumentation', 'N/A')}
- Music Identity: {previous_caption.get('music_identity_signature', 'N/A')}
- Energy: {previous_caption.get('energy', 'N/A')}

Compare the current audio segment with these characteristics to determine if it's a continuation of the same track.
...
def process_vlog_bgm(video_caption_json_path: str, bgm_audio_path: str, output_json_path: str):
    ...
    results = []
    previous_caption = None  # 追蹤前一個片段的 caption
    ...
    for idx, scene in enumerate(scenes, 1):
        ...
        # 使用 Gemini 生成 caption，傳入前一個片段的資訊
        print(f"  🤖 Generating audio caption with Gemini...")
        caption = caption_audio_with_gemini(segment_path, previous_caption)
        ...
        if caption:
            ...
            print(f"    Continuation: {caption.get('is_continuation', False)}")
            
            # 更新 previous_caption 供下一個片段使用
            previous_caption = caption
        else:
            print(f"  ✗ Failed to generate caption")
            previous_caption = None  # 如果失敗，重置
    ...
    # 統計連續的 BGM 片段
    continuations = sum(1 for r in results if r['audio_caption'].get('is_continuation', False))
    print(f"📊 Statistics: {continuations} segments are continuations of previous BGM")
